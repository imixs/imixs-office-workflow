...
public class ChronicleController implements Serializable {
	...
	protected void buildChronicle() {
		...
		if (!workflowController.getWorkitem().hasItem(HistoryPlugin.ITEM_HISTORY_LOG)
			...
			history = new ArrayList<>(workflowController.getWorkitem().getItemValue(HistoryPlugin.ITEM_HISTORY_LOG));
		}
		...
		logger.fine("...build chronicle in " + (System.currentTimeMillis() - l) + "ms");
	}
}
