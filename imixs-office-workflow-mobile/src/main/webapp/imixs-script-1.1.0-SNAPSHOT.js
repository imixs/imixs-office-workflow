 ...
 * 
 * @param service -
 *            rest uri to be called ( workflow/worklistbyowner/null)
 * @param selector -
	...
	if (data == null) {
		...
		if (serviceURL.indexOf('?') > -1)
			...
			serviceURL = serviceURL + "?";
				...
				// add filter
				serviceURL += "count=30&start=0&items=$uniqueid,txtworkflowgroup,txtworkflowstatus,txtworkflowsummary,txtworkflowimageurl,namcurrenteditor,$modified",
				...
				// start : "0",
				// items :
				// "$uniqueid,txtworkflowgroup,txtworkflowstatus,txtworkflowsummary,txtworkflowimageurl,namcurrenteditor,$modified"
				// }, function(data) {
 ...
 * 
 * The method supports two different json formats
 * 
 * Jersey:
 * 
 * <code>
		...
		if (item !=null && item.name == itemname) {
			if (item.value != null) {
				...
				if ($.isArray(item.value)) {
					if (item.value[0] !=null && typeof item.value[0] === 'object' )
						value = item.value[0].$;
					else
						value = item.value[0];
