...
function insertTextAtCursor(text) {
    ...
    if (selection.rangeCount === 0) {
        // No selection, append to editor as paragraph
    ...
    lines.forEach((line, index) => {
        if (line.trim()) {
            // Non-empty line - create text node
            if (index === 0) {
                // First line - insert as text node
                fragment.appendChild(document.createTextNode(line));
            } else {
                // Subsequent lines - wrap in paragraph
                const p = document.createElement('p');
                p.textContent = line;
                fragment.appendChild(p);
            }
        } else if (index < lines.length - 1) {
            // Empty line in middle - create line break
